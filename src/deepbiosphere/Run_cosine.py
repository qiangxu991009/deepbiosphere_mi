...
def save_model(model, optimizer, epoch, args, steps, scheduler=None):
    
    model_path= f"{paths.MODELS}{args.model}_{args.loss}/cosine_annealing/{args.exp_id}_lr{str(args.lr).split('.')[-1]}_e{epoch}.tar"
...
def train_model(args, rng):
    ...
    for epoch in range(args.epochs):
        ...
        save_model(model, optimizer, epoch, args, steps, scheduler)
        if scheduler is not None:
            scheduler.step()
...
if __name__ == "__main__":
    ...
    args.add_argument('--use_cosine_annealing', action='store_true', help='Use CosineAnnealingLR scheduler')
